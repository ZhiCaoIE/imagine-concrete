...
for storyline in storylines:
    ...
    for scenario_index in lever_list_story:
        ...
        #unit: 10,000 m2; CB: Concrete-brick
        new_m2_Res_Rural_CB = data_inputs.iloc[0:87,0]

        #unit: 10,000 m2; BT: Brick-timber
        new_m2_Res_Rural_BT = data_inputs.iloc[0:87,1]

        #unit: 10,000 m2; TO: Timber-others
        new_m2_Res_Rural_TO = data_inputs.iloc[0:87,2]

        ##################new residential urban floor area####################################
        #unit: 10,000 m2; CS: Concrete-steel
        new_m2_Res_Urban_CS = data_inputs.iloc[0:87,3]

        #unit: 10,000 m2; CB: Concrete-brick
        new_m2_Res_Urban_CB = data_inputs.iloc[0:87,4]

        #unit: 10,000 m2; BT: Brick-timber
        new_m2_Res_Urban_BT = data_inputs.iloc[0:87,5]

        ##################new nonresidential floor area########################################
        #unit: 10,000 m2; CS: Concrete-steel
        new_m2_NonR_CS = data_inputs.iloc[0:87,6]

        #unit: 10,000 m2; CB: Concrete-brick
        new_m2_NonR_CB = data_inputs.iloc[0:87,7]

        #unit: 10,000 m2; BT: Brick-timber
        new_m2_NonR_BT = data_inputs.iloc[0:87,8]
        ...
        #unit: 10,000 m2; in-use residential floor area
        Use_m2_Res = pop*m2_per_Res/10

        #unit: million m2; in-use non-residential floor area
        Use_m2_NonR = data_inputs.iloc[86:130,11]

        #unit: 10,000 m2
        Use_m2_NonR = Use_m2_NonR*100

        #Res-Urban and Res-Rural
        Res_Urban = data_inputs.iloc[86:130,12]
        Res_Rural = data_inputs.iloc[86:130,13]

        #Res_Rural_CB; Res_Rural_BT; Res_Rural_TO
        Res_Rural_CB = data_inputs.iloc[86:130,14]
        Res_Rural_BT = data_inputs.iloc[86:130,15]
        Res_Rural_TO = data_inputs.iloc[86:130,16]

        #Res_Urban_CS; Res_Urban_CB; Res_Urban_BT
        Res_Urban_CS = data_inputs.iloc[86:130,17]
        Res_Urban_CB = data_inputs.iloc[86:130,18]
        Res_Urban_BT = data_inputs.iloc[86:130,19]

        #NonR_CS; NonR_CB; NonR_BT
        NonR_CS = data_inputs.iloc[86:130,20]
        NonR_CB = data_inputs.iloc[86:130,21]
        NonR_BT = data_inputs.iloc[86:130,22]

        #lifetime: years; cement intensity: kg cement/m2
        Life = data_inputs.iloc[:,23:25]

        int_Con_Res_Rural_CB = data_inputs.iloc[:,25]
        int_Con_Res_Rural_BT = data_inputs.iloc[:,26]
        int_Con_Res_Rural_TO = data_inputs.iloc[:,27]
        int_Con_Res_Urban_CS = data_inputs.iloc[:,28]
        int_Con_Res_Urban_CB = data_inputs.iloc[:,29]
        int_Con_Res_Urban_BT = data_inputs.iloc[:,30]
        int_Con_NonR_CS = data_inputs.iloc[:,31]
        int_Con_NonR_CB = data_inputs.iloc[:,32]
        int_Con_NonR_BT = data_inputs.iloc[:,33]
        ...
        #assemble data series into dataframe
        new_m2_matrix = pd.concat([new_m2_Res_Rural_CB, new_m2_Res_Rural_BT, new_m2_Res_Rural_TO,\
            new_m2_Res_Urban_CS, new_m2_Res_Urban_CB, new_m2_Res_Urban_BT,\
                new_m2_NonR_CS, new_m2_NonR_CB, new_m2_NonR_BT], axis = 1)
        ...
        int_Con_matrix = pd.concat([int_Con_Res_Rural_CB*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015),\
                                    ...
                                    int_Con_Res_Urban_CS*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015),\
                                    ...
                                    int_Con_NonR_CS*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015),\
                                    int_Con_NonR_CB*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015),\
                                    int_Con_NonR_BT*(1-Leaner_con_saving)*(1)*(1-waste_saving*0.015)], axis = 1)

        int_Tim_matrix = pd.concat([int_Con_Res_Rural_CB*(1-Leaner_con_saving)*Mat_sub_rate*Timber_to_cem_res,\
                                    int_Con_Res_Rural_BT*(1-Leaner_con_saving)*0,\
                                    int_Con_Res_Rural_TO*(1-Leaner_con_saving)*0,\
                                    int_Con_Res_Urban_CS*(1-Leaner_con_saving)*Mat_sub_rate*Timber_to_cem_res,\
                                    int_Con_Res_Urban_CB*(1-Leaner_con_saving)*Mat_sub_rate*Timber_to_cem_res,\
                                    int_Con_Res_Urban_BT*(1-Leaner_con_saving)*0,\
                                    int_Con_NonR_CS*(1-Leaner_con_saving)*Mat_sub_rate*Timber_to_cem_nonr,\
                                    int_Con_NonR_CB*(1-Leaner_con_saving)*Mat_sub_rate*Timber_to_cem_nonr,\
                                    int_Con_NonR_BT*(1-Leaner_con_saving)*0], axis = 1)
        ...
        int_Cem_Res_Urban_CS = int_Con_Res_Urban_CS*cemen_content*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015)*(1-CO2_curing_sav-CO2_mine_EOL_sav)
        ...
        int_Cem_NonR_CS = int_Con_NonR_CS*cemen_content*(1-Leaner_con_saving)*(1-Mat_sub_rate)*(1-waste_saving*0.015)*(1-CO2_curing_sav-CO2_mine_EOL_sav)
        ...
        int_Cem_matrix = pd.concat([int_Cem_Res_Rural_CB, int_Cem_Res_Rural_BT, int_Cem_Res_Rural_TO,\
            int_Cem_Res_Urban_CS, int_Cem_Res_Urban_CB, int_Cem_Res_Urban_BT,\
                int_Cem_NonR_CS, int_Cem_NonR_CB, int_Cem_NonR_BT], axis = 1)

        #Use_m2_Res_Urban_CB; unit: 10,000 m2
        ...
        Use_m2_Res_Urban_CS = Use_m2_Res * Res_Urban * Res_Urban_CS
        Use_m2_Res_Urban_CB = Use_m2_Res * Res_Urban * Res_Urban_CB
        Use_m2_Res_Urban_BT = Use_m2_Res * Res_Urban * Res_Urban_BT

        Use_m2_NonR_CS = Use_m2_NonR * NonR_CS
        Use_m2_NonR_CB = Use_m2_NonR * NonR_CB
        Use_m2_NonR_BT = Use_m2_NonR * NonR_BT

        Use_m2_matrix = pd.concat([Use_m2_Res_Rural_CB, Use_m2_Res_Rural_BT, Use_m2_Res_Rural_TO,\
            Use_m2_Res_Urban_CS, Use_m2_Res_Urban_CB, Use_m2_Res_Urban_BT,\
                Use_m2_NonR_CS, Use_m2_NonR_CB, Use_m2_NonR_BT], axis = 1)
        ...
        for i in range(0, len(new_m2_matrix.columns)):
            ...
            for k in range(2018, 2061):
                ...
                demolish_Con_list = demolish_m2_list * int_Con * 10000/1000
                demolish_Con      = sum(demolish_Con_list)
                demol_Con_extended= np.append(demol_Con_extended,demolish_Con)

                demolish_Tim_list = demolish_m2_list * int_Tim * 10000/1000
                ...
                reuse_Con_list    = demolish_m2_list * int_Con * 10000/1000 * reuse_rate
                reuse_Cem_list    = demolish_m2_list * int_Cem * 10000/1000 * reuse_rate
            ...
            con_app = new_m2_extended * int_Con_matrix.iloc[:,i] * 10000/1000 - reuse_Con_extended
            con_app_matrix = pd.concat([con_app_matrix, pd.Series(con_app)], axis = 1, ignore_index = True)

            cem_app = new_m2_extended * int_Cem_matrix.iloc[:,i] * 10000/1000 - reuse_Cem_extended
            cem_app_matrix = pd.concat([cem_app_matrix, pd.Series(cem_app)], axis = 1, ignore_index = True)

            tim_con = new_m2_extended * int_Tim_matrix.iloc[:,i] * 10000/1000
        ...
        pd.DataFrame(new_m2_extended_matrix).to_excel(
            r'C:\Users\caozh\Documents\2020-ClimateWork\Carbonation\China-Buildings' + \
